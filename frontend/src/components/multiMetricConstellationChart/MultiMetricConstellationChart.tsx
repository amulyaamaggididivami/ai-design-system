...
import type { MultiMetricConstellationChartProps } from './types';
...
export function MultiMetricConstellationChart({ items: rawItems = [], 'data-testid': testId }: MultiMetricConstellationChartProps) {
  ...
  const constellations = useMemo(() => {
    const maxBase = Math.max(...items.map(c => c.base ?? 0));
    const maxVar  = Math.max(...items.map(c => c.variation ?? 0));

    return items.map((con, pi) => {
